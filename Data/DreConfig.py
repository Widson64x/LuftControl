...
from flask import Blueprint, jsonify, request, abort, render_template, json
...
"""
Teste para verificar existência da tabela DRE_Estrutura_No_Virtual
Cole este código temporariamente em uma rota ou execute separadamente
"""

# Adicione esta rota temporária no seu DreConfig.py para testar:

@dre_config_bp.route('/Teste/VerificarTabelaNoVirtual', methods=['GET'])
@login_required
def teste_tabela_no_virtual():
    """
    Testa a existência da tabela de duas formas:
    1. Query SQL direta
    2. SQLAlchemy ORM
    """
    session = get_session()
    ...
    try:
        ...
        # ========== TESTE 2: Listar todas as tabelas do schema ==========
        sql_todas = text("""
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'Dre_Schema'
        ...
        # ========== TESTE 3: SQLAlchemy ORM - Tentar query ==========
        try:
        ...
        # ========== TESTE 4: Verificar nome exato da tabela (case sensitive) ==========
        sql_case = text("""
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'Dre_Schema' 
        ...
        # ========== TESTE 5: Verificar estrutura da tabela se existir ==========
        sql_colunas = text("""
...
def AddNoCalculado():
    ...
    Body JSON:
    {
        ...
        "ordem": 50,
        "tipo_exibicao": "valor",
        "estilo_css": "font-weight: bold; color: #2ecc71;"
    }
    """
    session = get_session()
    try:
...
def GetOperandosDisponiveis():
    """
    Retorna lista de todos os elementos que podem ser usados como operandos
    em fórmulas (nós virtuais, tipos CC, subgrupos de primeiro nível).
    """
    session = get_session()
    try:
        print("Atributos do DreNoVirtual:", dir(DreNoVirtual))
        resultado = {
            "nos_virtuais": [],
            "tipos_cc": [],
            "subgrupos_raiz": []
        ...
        # 1. Nós Virtuais (incluindo calculados)
        ...
        for row in tipos:
            tipo_valor = row[0] # Pega o primeiro valor da tupla/linha
            resultado["tipos_cc"].append({
                "id": tipo_valor,
                "nome": tipo_valor
            })
        
        # 3. Subgrupos de primeiro nível (opcionalmente)
