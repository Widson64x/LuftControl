...
def RelatorioRentabilidade():
    ...
    try: 
        ...
        sql_query = text(f"""
            ...
            FROM DadosBrutos
            GROUP BY "Tipo_CC", "Root_Virtual_Id", "Caminho_Subgrupos", "Titulo_Final"
        ...
        sql_nos_calculados = text("""
            SELECT 
                ...
                nv."Estilo_CSS", nv."Tipo_Exibicao",
                COALESCE(ord.ordem, 999) as ordem_prioridade
            FROM "Dre_Schema"."DRE_Estrutura_No_Virtual" nv
            LEFT JOIN "Dre_Schema"."DRE_Ordenamento" ord 
                ON ord.id_referencia = CAST(nv."Id" AS TEXT) 
                AND ord.contexto_pai = 'root'
        ...
        for no in nos_calculados:
            ...
            try:
                ...
                for mes in meses:
                    ...
                    elif operacao == 'subtracao': resultado = valores_ops[0] - sum(valores_ops[1:])
                    elif operacao == 'multiplicacao': 
                        import math
                        resultado = math.prod(valores_ops)
                    elif operacao == 'divisao': 
                        resultado = valores_ops[1] != 0 and valores_ops[0] / valores_ops[1] or 0.0

                    multiplicador = float(formula.get('multiplicador', 1))
                ...
                print(f"Erro ao calcular nó {no.Nome}: {e}")
                continue
