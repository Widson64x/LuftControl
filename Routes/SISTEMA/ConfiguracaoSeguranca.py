...
from Models.SqlServer.Permissoes import Tb_Permissao, Tb_PermissaoGrupo, Tb_PermissaoUsuario
from Models.SqlServer.Usuario import UsuarioGrupo, Usuario
from Modules.SISTEMA.Services.PermissaoService import RequerPermissao, PermissaoService, DEBUG_PERMISSIONS
...
def VisualizarGerenciador():
    ...
    try:
        ...
        ListaPermissoes = Sessao.query(Tb_Permissao).filter_by(Id_Sistema=SISTEMA_ID).order_by(
            Tb_Permissao.Categoria_Permissao,
            Tb_Permissao.Descricao_Permissao
        ).all()

        PermissoesPorCategoria = {}
        for permissao in ListaPermissoes:
            categoria = permissao.Categoria_Permissao or 'Geral'
            if categoria not in PermissoesPorCategoria:
                PermissoesPorCategoria[categoria] = []
            PermissoesPorCategoria[categoria].append(permissao)
        ...
        return render_template(
            'Pages/Configs/PermissionConfigs.html',
            Grupos=Grupos,
            Usuarios=Usuarios,
...
def BuscarAcessosUsuario():
    ...
    try:
        ...
        ListaGrupo = [p.Id_Permissao for p in Sessao.query(Tb_PermissaoGrupo).filter_by(Codigo_UsuarioGrupo=User.codigo_usuariogrupo).all()]
