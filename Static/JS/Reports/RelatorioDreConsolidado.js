...
class RelatorioDreConsolidado {
    ...
    processTree() {
        ...
        const sumValues = (node, row) => {
            colunas.forEach(c => node.values[c] += (parseFloat(row[c]) || 0));
        };
        ...
        this.rawData.forEach((row, index) => {
            ...
            colunas.forEach(c => {
                contaNode.values[c] += (parseFloat(row[c]) || 0);
            });
        });
    ...
    async processTreeWithCalculated() {
        ...
        if (nosCalc.length > 0) {
            ...
            nosCalc.forEach(noCalc => {
                ...
                const valores = {};
                colunas.forEach(col => {
                    valores[col] = this.calcularValorNo(noCalc.formula, col, valoresAgregados, contextoSuffix);
                });
    ...
    calcularValorNo(formula, mes_ou_coluna, valoresAgregados, contextoSuffix = null) {
        ...
        const valores = formula.operandos.map(op => {
            ...
            if (tipoOp === 'conta' && contextoSuffix) {
                ...
                if (valoresAgregados[chaveEspecifica] && valoresAgregados[chaveEspecifica][mes_ou_coluna] !== undefined) return valoresAgregados[chaveEspecifica][mes_ou_coluna];
            }
    ...
    agregarValoresPorTipo() {
        ...
        const initKey = (key) => {
            if (!agregados[key]) {
                agregados[key] = {};
                colunas.forEach(c => agregados[key][c] = 0);
            }
        };
        ...
        const traverse = (node) => {
            if (node.type === 'root') {
                ...
                initKey(keyTipo);
                colunas.forEach(c => agregados[keyTipo][c] += (node.values[c] || 0));
            }

            if (node.virtualId) {
                const keyVirt = `no_virtual_${node.virtualId}`;
                initKey(keyVirt);
                colunas.forEach(c => agregados[keyVirt][c] += (node.values[c] || 0));
            }

            if (node.type === 'group') {
                const keySub = `subgrupo_${node.label}`;
                initKey(keySub);
                colunas.forEach(c => agregados[keySub][c] += (node.values[c] || 0));
            }
            
            if (node.type === 'account' && node.contaCodigo) {
                const keyPura = `conta_${node.contaCodigo}`;
                initKey(keyPura);
                colunas.forEach(c => agregados[keyPura][c] += (node.values[c] || 0));
                ...
                if (node.tipoCC) {
                    ...
                    if (['Coml', 'Oper', 'Adm'].includes(tipoClean)) {
                        ...
                        initKey(keyComposta);
                        colunas.forEach(c => agregados[keyComposta][c] += (node.values[c] || 0));
                    }
                }
            }
