/**
 * Motor de Grid de Ajustes do Razão Contábil
 * Responsável pela renderização virtualizada, filtros de excel, árvore de datas
 * e interações de contexto (edição, aprovação, invalidação).
 */
const GridAjustes = {
    rawData: [], 
    viewData: [], 
    ...
    /**
     * Instancia e exibe uma notificação flutuante no canto da tela, reportando também ao console.
     * * @param {string} msg Mensagem a ser renderizada.
     * @param {string} type Tipo da notificação ('success', 'warning', 'error').
     */
    exibirNotificacao: function(msg, type = 'success') {
        console.log(`[Notificação - ${type.toUpperCase()}] ${msg}`);
    ...
    /**
     * Orquestra a injeção do script, efetuando cache dos elementos e preparando carregamentos paralelos.
     */
    inicializar: function() {
        console.log("[GridAjustes] Processo de inicialização requisitado.");
        try {
            this.mapearDom();
            ...
            this.carregarDados();
            console.log("[GridAjustes] Sistema inicializado e pronto para operar.");
        } catch (error) {
            console.error("[GridAjustes] Falha crítica no arranque da classe:", error);
            this.exibirNotificacao("Erro fatal ao iniciar a tela. Analise o console.", "error");
        }
    },

    /**
     * Localiza os nós HTML e aloca propriedades referenciáveis em memória.
     */
    mapearDom: function() {
        console.log("[GridAjustes] Mapeando estrutura DOM...");
        this.dom = {
        ...
        for (const [chave, elemento] of Object.entries(this.dom)) {
            if (!elemento && chave !== 'btnInter') {
                console.warn(`[GridAjustes] O elemento '${chave}' não foi encontrado no HTML.`);
            }
        }
    },

    /**
     * Agrupa os Listeners de eventos de clique, teclado e scroll.
     */
    vincularEventos: function() {
        console.log("[GridAjustes] Vinculando escutas de eventos (Listeners).");
        this.dom.tableBody.addEventListener('click', (e) => this.manipularCliqueGrid(e));
        this.dom.tableBody.addEventListener('contextmenu', (e) => this.manipularMenuContexto(e));
        
        this.dom.tableHead.addEventListener('input', (e) => {
            if(e.target.classList.contains('luft-filter-input')) this.manipularInputFiltro(e.target);
        });

        this.dom.btnLoad.onclick = () => this.carregarDados();
        this.dom.btnAdd.onclick = () => this.adicionarNovaLinha();
        this.dom.btnCsv.onclick = () => this.exportarCsv();
        ...
        if (container) {
            container.addEventListener('scroll', () => requestAnimationFrame(() => this.renderizarCorpo()));
        }
        ...
        this.dom.ctxMenu.addEventListener('click', (e) => {
            ...
            if(li && li.dataset.action) {
                this.executarAcaoContexto(li.dataset.action);
    ...
    /**
     * Efetua requisição assíncrona contra o controlador para buscar os dados de grade,
     * aplicando proteções de desempacotamento seguro em arrays.
     */
    carregarDados: async function() {
        console.log("[GridAjustes] Invocando carregamento de dados...");
        this.alternarCarregamento(true);
        try {
            const ano = this.dom.tbAno.value;
            const mes = this.dom.tbMes.value;
            
            console.log(`[GridAjustes] Parâmetros capturados: Mês=${mes}, Ano=${ano}`);
            if (typeof APIUtils === 'undefined') {
                throw new Error("Dependência APIUtils não detectada no contexto global.");
            }

            const resposta = await APIUtils.get(`${API.getDados}?ano=${ano}&mes=${mes}`);
            console.log("[GridAjustes] Resposta original da API:", resposta);
            
            const arrayExtraido = Array.isArray(resposta) ? resposta : (resposta && resposta.data ? resposta.data : []);
            console.log(`[GridAjustes] ${arrayExtraido.length} itens extraídos validados.`);
            
            this.rawData = arrayExtraido;
            this.excelFilters = {}; 
            this.filters = {};
            document.querySelectorAll('.luft-filter-input').forEach(i => i.value = '');
            
            this.aplicarFiltrosGlobais(); 
            this.atualizarStatus(`${this.rawData.length} registros contabilizados no cliente.`);
        } catch (e) { 
            console.error("[GridAjustes] Exceção gerada no carregarDados:", e);
            this.exibirNotificacao(e.message, 'error'); 
        } finally { 
            this.alternarCarregamento(false); 
        }
    },

    /**
     * Reconstrói as colunas de cabeçalho baseando-se na definição da matriz Columns.
     */
    renderizarCabecalho: function() {
        let html = '<th class="luft-col-index">#</th>';
        this.columns.forEach(col => {
            const iconFilter = `<i class="fas fa-filter luft-filter-icon" onclick="GridAjustes.abrirFiltro('${col.key}', event)"></i>`;
            html += `
    ...
    /**
     * Efetua conversões robustas para objetos de data mitigando strings incompletas ou fuso horário indesejado.
     * * @param {string|Date} val Valor originário contendo a data.
     * @returns {Date|null} O objeto Date validado ou null.
     */
    interpretarDataSegura: function(val) {
        if (!val) return null;
    ...
    /**
     * Projeta um menu popup de seleção múltipla (estilo Excel) posicionado no viewport.
     * * @param {string} key Chave referencial da coluna.
     * @param {Event} e O evento de cursor capturado.
     */
    abrirFiltro: function(key, e) {
        e.stopPropagation();
        ...
        if (colDef.type === 'date') {
            ...
            this.rawData.forEach(row => {
                const dataObj = this.interpretarDataSegura(row[key]);
                if (!dataObj) return;
            ...
            Object.keys(tree).sort((a,b) => b - a).forEach(year => {
                listHtml += `
                    <li class="tree-node"><span class="luft-caret" onclick="GridAjustes.alternarArvore(this)">▶</span> 
                    <input type="checkbox" class="luft-chk" data-type="year" value="${year}" onclick="GridAjustes.manipularCliqueCheckbox(this)"> <b>${year}</b></li><ul id="group-${year}">`;
                Object.keys(tree[year]).sort((a,b) => a - b).forEach(mesIdx => {
                    const valMes = `${year}-${parseInt(mesIdx)+1}`;
                    listHtml += `
                        <li class="tree-node"><span class="luft-caret" onclick="GridAjustes.alternarArvore(this)">▶</span> 
                        <input type="checkbox" class="luft-chk" data-type="month" data-parent="${year}" value="${valMes}" onclick="GridAjustes.manipularCliqueCheckbox(this)"> ${mesesPT[mesIdx]}</li><ul id="group-${valMes}">`;
                    tree[year][mesIdx].sort((a,b) => a - b).forEach(dia => {
                        ...
                        listHtml += `<li class="tree-leaf"><input type="checkbox" class="luft-chk leaf" data-type="day" data-parent="${valMes}" data-root="${year}" value="${fullDate}" ${checkedAttr} onclick="GridAjustes.manipularCliqueCheckbox(this)"> ${strDia}</li>`;
                    });
                    listHtml += `</ul>`;
                });
                listHtml += `</ul>`;
            ...
            listHtml += `<li><input type="checkbox" class="luft-chk" id="chk-all" ${allSelected ? 'checked' : ''} onclick="GridAjustes.alternarTodos(this)"> <b>(Selecionar Tudo)</b></li>`;
            uniqueValues.forEach(v => {
                const strVal = String(v);
                const checkedAttr = (!activeFilter || activeFilter.includes(strVal)) ? 'checked' : '';
                listHtml += `<li><input type="checkbox" class="luft-chk leaf simple" value="${strVal}" ${checkedAttr} onclick="GridAjustes.manipularCliqueUnico(this)"> ${v}</li>`;
            });
        }

        menu.innerHTML = `
            <div class="luft-filter-header"><input type="text" class="luft-filter-search" placeholder="Pesquisar..." onkeyup="GridAjustes.filtrarLista(this)"></div>
            ...
            <div class="luft-filter-footer">
                ...
                <button class="luft-btn-ok" onclick="GridAjustes.aplicarFiltroExcel('${key}', '${colDef.type}')">Aplicar</button>
            </div>
        `;

        document.body.appendChild(menu);
        this.atualizarEstadoMenu(menu);
        ...
        setTimeout(() => {
            document.addEventListener('click', function fecharMenu(ev) {
                if(!menu.contains(ev.target) && ev.target !== e.target && !ev.target.classList.contains('luft-filter-icon')) {
                    menu.remove(); document.removeEventListener('click', fecharMenu);
                }
            });
        }, 100);
    },

    atualizarEstadoMenu: function(menu) {
        menu.querySelectorAll('input[data-type="month"]').forEach(cb => this.atualizarEstadoPai(cb.value, menu));
        menu.querySelectorAll('input[data-type="year"]').forEach(cb => this.atualizarEstadoPai(cb.value, menu));
    ...
    manipularCliqueCheckbox: function(cb) {
        ...
        else if (type === 'month') { menu.querySelectorAll(`input[data-parent="${val}"]`).forEach(c => c.checked = cb.checked); this.atualizarEstadoPai(cb.dataset.parent, menu); }
        else if (type === 'day') { this.atualizarEstadoPai(cb.dataset.parent, menu); const pm = menu.querySelector(`input[value="${cb.dataset.parent}"]`); if(pm) this.atualizarEstadoPai(pm.dataset.parent, menu); }
    },

    atualizarEstadoPai: function(parentVal, menu) {
    ...
    aplicarFiltroExcel: function(key) {
    ...
    manipularInputFiltro: function(input) {
    ...
    aplicarFiltrosGlobais: function() {
        console.log("[GridAjustes] Executando pipeline de filtros...");
        if (Object.keys(this.filters).length === 0 && Object.keys(this.excelFilters).length === 0) {
            ...
            this.viewData = this.rawData.filter(row => {
                ...
                return Object.keys(this.excelFilters).every(k => {
                    ...
                    if(colDef.type === 'date') {
                        const d = this.interpretarDataSegura(rowVal);
                        if(!d) return false;
        ...
        console.log(`[GridAjustes] Filtros concluídos. Total visível: ${this.viewData.length}`);
        this.renderizarCorpo(); 
        this.atualizarStatus(`${this.viewData.length} linhas exibidas.`);
    },

    alternarTodos: function(cb) { 
        cb.closest('.luft-excel-filter-menu').querySelectorAll('.luft-chk.simple').forEach(c => c.checked = cb.checked); 
    },
    
    manipularCliqueUnico: function(cb) {
    ...
    /**
     * Aplica o algoritmo de virtualização para inserir no DOM apenas as linhas vísiveis em tela.
     */
    renderizarCorpo: function() {
        const container = document.querySelector('.luft-grid-container');
        if (!container) return;
        ...
        if (endIndex < 0) {
            this.dom.tableBody.innerHTML = `<tr><td colspan="${this.columns.length + 1}" class="text-center text-muted">Nenhum dado a exibir.</td></tr>`;
            this.renderizarRodape(0, 0);
            return;
        }
        ...
        for (let i = startIndex; i <= endIndex; i++) {
            ...
            this.columns.forEach(col => {
                ...
                html += `<td class="${cls} ${stCls}" data-key="${col.key}">${this.formatarValor(val, col.type)}</td>`;
            });
            html += '</tr>';
        }
        ...
        this.renderizarRodape(tDeb, tCred);
    },

    renderizarRodape: function(deb, cred) {
        const saldo = deb - cred;
        const color = saldo < 0 ? 'var(--luft-danger-600)' : 'var(--luft-primary-600)';
        let html = '<tr><td class="luft-col-index">Σ</td>';
        this.columns.forEach(col => {
            if(col.key === 'Debito') html += `<td class="text-right text-money">${this.formatarDinheiro(deb)}</td>`;
            else if(col.key === 'Credito') html += `<td class="text-right text-money">${this.formatarDinheiro(cred)}</td>`;
            else if(col.key === 'Saldo') html += `<td class="text-right text-money" style="color:${color}">${this.formatarDinheiro(saldo)}</td>`;
            else html += '<td></td>';
        });
        html += '</tr>';
    ...
    manipularCliqueGrid: function(e) {
        ...
        if (colDef.type === 'bool') { rowData[key] = !rowData[key]; this.salvarLinha(rowData); return; }
        this.iniciarEdicao(td, idx, key, colDef);
    },

    iniciarEdicao: function(td, idx, key, colDef) {
        this.isEditing = true; 
        const row = this.viewData[idx], val = row[key];
        const input = document.createElement('input'); 
        input.className = 'luft-cell-editor';
        const dateSafe = this.interpretarDataSegura(val);
    ...
    manipularMenuContexto: function(e) {
        e.preventDefault(); 
    ...
    executarAcaoContexto: async function(action) {
        if (this.ctxIndex < 0) return;
        const row = this.viewData[this.ctxIndex], id = row.Id, fonte = row.Fonte;
        
        if (action === 'HISTORICO') { this.abrirHistorico(id, fonte); return; }
        
        if (!id || !fonte) {
            this.exibirNotificacao("Efetue a gravação do registro em banco antes de aplicar regras de aprovação.", "warning");
            return;
        }

        console.log(`[GridAjustes] Despachando Ação de Contexto: ${action}`);
        this.alternarCarregamento(true);
        try {
            ...
            this.carregarDados(); 
            this.exibirNotificacao("Alteração de estado aplicada com sucesso.");
        } catch (e) { 
            console.error("[GridAjustes] Erro ao aplicar ação contextual:", e);
            this.exibirNotificacao(e.message, "error"); 
        } finally { 
            this.alternarCarregamento(false); 
        }
    ...
    adicionarNovaLinha: function() {
        console.log("[GridAjustes] Inserindo buffer manual de nova linha vazia.");
    ...
    salvarLinha: async function(row) {
        console.log("[GridAjustes] Sincronizando edição de linha com o banco de dados...");
        try {
            const isCriacao = (row.Tipo_Linha === 'Inclusao' && !row.Id);
            const payload = { ...row }; 
            if (isCriacao) delete payload.Id;
            
            const urlAlvo = isCriacao ? API.criar : API.salvar;
            const res = await APIUtils.post(urlAlvo, { Tipo_Operacao: isCriacao ? 'INCLUSAO' : 'EDICAO', Dados: payload, Id: row.Id, Fonte: row.Fonte });
            
            if (res && res.id) { 
                row.Id = res.id; 
                if (row.Status_Ajuste === 'Original' || !row.Status_Ajuste) row.Status_Ajuste = 'Pendente'; 
                this.exibirNotificacao(isCriacao ? "Sincronização (Create) executada." : "Sincronização (Update) executada.");
            }
        } catch (e) { 
            console.error("[GridAjustes] Erro durante a tentativa de salvar a linha:", e);
            this.exibirNotificacao(e.message, "error"); 
        } finally { 
            this.renderizarCorpo(); 
        }
    ...
    gerarIntergrupo: async function() {
        ...
        if (!ano || !mes) return this.exibirNotificacao("Os parâmetros Ano e Mês são obrigatórios para esta rotina.", "warning");
        if(!confirm(`Confirma o processamento assíncrono de intergrupos para o período de ${mes}/${ano}?`)) return;
        
        console.log(`[GridAjustes] Transação Intergrupo Desencadeada para: ${mes}/${ano}`);
        this.alternarCarregamento(true);
        try {
            const result = await APIUtils.post(API.intergrupo, { ano: parseInt(ano), mes: parseInt(mes) });
            
            const arraysLogs = Array.isArray(result) ? result : (result && result.data ? result.data : []);
            
            this.exibirNotificacao(`Transação concluída. ${arraysLogs.length} logs emitidos pelo backend.`, "success");
            this.carregarDados();
        } catch(e) { 
            console.error("[GridAjustes] Erro severo na geração do intergrupo:", e);
            this.exibirNotificacao(e.message, "error"); 
        } finally { 
            this.alternarCarregamento(false); 
        }
    ...
    abrirHistorico: async function(id, fonte) {
        if (!id || !fonte) return;
        console.log(`[GridAjustes] Consultando Histórico Auditável. ID=${id}, Fonte=${fonte}`);
        try {
            const result = await APIUtils.get(`${API.historico}?id=${id}&fonte=${fonte}`);
            const dadosHistorico = Array.isArray(result) ? result : (result && result.data ? result.data : []);
            
            document.getElementById('historyBody').innerHTML = dadosHistorico.map(l => `<tr><td>${l.Data}</td><td>${l.Usuario}</td><td>${l.Campo}</td><td class="text-danger">${l.De}</td><td class="text-success font-bold">${l.Para}</td></tr>`).join('');
            document.getElementById('modalHistory').classList.add('show');
        } catch (e) {
            console.error("[GridAjustes] Impossível detalhar histórico auditável:", e);
            this.exibirNotificacao(e.message, "error");
        }
    },

    formatarValor: function(val, type) {
        if (val === null || val === undefined) return '';
        if (type === 'money') return parseFloat(val).toLocaleString('pt-BR', {minimumFractionDigits: 2});
        if (type === 'date') { const d = this.interpretarDataSegura(val); return d ? d.toLocaleDateString('pt-BR') : val; }
    ...
    formatarDinheiro: (v) => parseFloat(v).toLocaleString('pt-BR', {minimumFractionDigits: 2}),
    
    alternarCarregamento: function(show) { 
        if(this.dom.loader) this.dom.loader.style.display = show ? 'flex' : 'none'; 
    },
    
    atualizarStatus: function(msg) { 
        if(this.dom.status) this.dom.status.textContent = msg; 
    },

    exportarCsv: function() {
        console.log("[GridAjustes] Montando Buffer de Blob CSV...");
        ...
        this.viewData.forEach(r => {
            const dataFormatada = r.Data ? this.interpretarDataSegura(r.Data).toLocaleDateString('pt-BR') : '';
        ...
        a.download = 'Razao_Contabil_Ajustes.csv'; 
        a.click();
    }
};

document.addEventListener('DOMContentLoaded', () => GridAjustes.inicializar());